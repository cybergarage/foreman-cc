...
namespace gorilla {
      ...
      int64_t end)
  {
    ...
    try {
      ...
      int64_t firstTimestamp = BitUtil::readValueFromBitString(data, bitPos, kBitsForFirstTimestamp);
      ...
      for (int i = 1; i < n; i++) {
        ...
        if (unixTime >= begin) {
          if (unixTime < FLAGS_gorilla_blacklisted_time_min || unixTime > FLAGS_gorilla_blacklisted_time_max) {
