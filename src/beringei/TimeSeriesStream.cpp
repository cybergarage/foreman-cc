...
namespace gorilla {
  ...
  void TimeSeriesStream::reset(int64_t minTimestamp, int64_t minTimestampDelta)
  {
    ...
    prevTimestamp_ = std::max(minTimestamp, minTimestampDelta) - minTimestampDelta;
  }
      ...
      int64_t minTimestampDelta)
  {
    ...
    for (int i = 0; i < 4; i++) {
      if (absValue < ((int64_t)1 << (timestampEncodings[i].bitsForValue - 1))) {
        ...
        int64_t encodedValue = deltaOfDelta + ((int64_t)1 << (timestampEncodings[i].bitsForValue - 1));
  ...
  void TimeSeriesStream::appendValue(double value)
  {
    ...
    if (leadingZeros >= previousValueLeadingZeros_ && trailingZeros >= previousValueTrailingZeros_ && previousBlockInformationSize < expectedSize) {
      ...
      int64_t& prevDelta)
  {
    ...
    if (type > 0) {
      ...
      decodedValue -= ((int64_t)1 << (timestampEncodings[index].bitsForValue - 1));
      if (decodedValue >= 0) {
      ...
      uint64_t& previousTrailingZeros)
  {
    ...
    uint64_t usePreviousBlockInformation = BitUtil::readValueFromBitString(data, bitPos, 1);
    ...
    if (usePreviousBlockInformation) {
      ...
      uint64_t leadingZeros = BitUtil::readValueFromBitString(data, bitPos, kLeadingZerosLengthBits);
      uint64_t blockSize = BitUtil::readValueFromBitString(data, bitPos, kBlockSizeLengthBits) + kBlockSizeAdjustment;
