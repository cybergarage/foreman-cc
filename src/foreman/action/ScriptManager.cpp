...
bool Foreman::Action::ScriptManager::execScript(const std::string& lang, const std::string& script, int encodeType, std::string* result, Error* err)
{
  const ScriptEngine* scriptEngine = this->engines.getEngine(lang);
  if (!scriptEngine) {
    FOREMANCC_ERROR_SET_ERRORNO(err, ERROR_INTERNAL_ERROR);
  ...
  return scriptEngine->run(script, result, err);
}

bool Foreman::Action::ScriptManager::execMethod(const std::string& name, const std::string& params, std::string* results, Error* err)
{
