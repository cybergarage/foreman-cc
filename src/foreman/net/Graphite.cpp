...
bool Graphite::addValue(const std::string& path, time_t ts, double value)
{
  ...
  if (std::std::snprintf(pickleMsg, sizeof(pickleMsg), "%s %ld %lf", path.c_str(), ts, value) < 0)
    return false;
...
bool Graphite::getValues(const std::string& path, time_t beginTs, time_t endTs, time_t interval, double** values, size_t* valueCnt)
{
  ...
  char sinceStr[32];
  tm = std::localtime(&beginTs);
  if (std::strftime(sinceStr, sizeof(sinceStr), "%H:%M_%Y%m%d", tm) == 0)
    return false;

  char endStr[32];
  tm = std::localtime(&endTs);
  if (std::strftime(endStr, sizeof(endStr), "%H:%M_%Y%m%d", tm) == 0)
    return false;

  char urlPath[256];
  if (std::snprintf(urlPath, sizeof(urlPath), "http://%s/render?target=%s&from=%s&until=%s&format=csv", host.c_str(), path.c_str(), sinceStr, endStr) < 0)
    return false;
